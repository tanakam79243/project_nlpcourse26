...
my_simple_corpus  <- quanteda::corpus(my_simple_sample, docid_field = "author", text_field = "doc_text")
...
kwic(md_tokenized, pattern = "ocean")
```
...
#Just messing around with another version of tokens
tokens_md2 <- tokens(md, remove_punct = TRUE) # %>% tokens_remove(stopwords("SMART"))
md_dfm2 <- dfm(tokens_md2) #removed , dfm_stem = TRUE from 
topfeatures(md_dfm2, 30)

# Create a document-feature matrix, applying stemming
md_dfm <- dfm(tokens_md, dfm_stem = TRUE)
topfeatures(md_dfm, 30)
...
quanteda.textplots::textplot_wordcloud(md_dfm, random_order = FALSE, rotation = 0.25, color = "#296E05")

 #word cloud or another way
